...
executar_comando() {
    ...
    if $cmd >"$temp_output" 2>&1; then
        ...
        [ "$results" -gt 0 ] && CHECKLIST+=("$name: ✓ $success_msg ($results linhas, ${elapsed}s)") || CHECKLIST+=("$name: ✓ $fail_msg (${elapsed}s)")
    else
        local end_time=$(date +%s)
        local elapsed=$((end_time - start_time))
...
test_http() {
    ...
    if curl -sI -o "$output_file" -w "%{http_code}" "$protocol://$TARGET" | grep -qE '200|301|302'; then
        local end_time=$(date +%s)
        local elapsed=$((end_time - start_time))
        CHECKLIST+=("HTTP ($protocol): ✓ Servidor ativo (${elapsed}s)")
    else
        local end_time=$(date +%s)
        local elapsed=$((end_time - start_time))
...
test_ffuf_directories() {
    if [ "$TYPE_TARGET" = "DOMAIN" ]; then
        ...
        for cmd in "${FFUF_WEB_COMMANDS[@]}"; do
            ...
            executar_comando "$cmd_substituido" "FFUF Web (iniciado às $start_time_display)" "$RESULTS_DIR/ffuf_web.csv" "Recursos web encontrados" "Nenhum recurso web encontrado"
        done
    else
...
Ativo_complexo() {
    ...
    if [ -n "$TARGET_IPv4" ]; then
        for cmd in "${NMAP_COMMANDS_IPV4[@]}"; do
            ...
            executar_comando "$cmd_substituido -oX $output_file" "Nmap IPv4 (iniciado às $start_time_display)" "$output_file" "Portas escaneadas" "Nenhuma porta encontrada"
    ...
    if [ -n "$TARGET_IPv6" ]; then
        for cmd in "${NMAP_COMMANDS_IPV6[@]}"; do
            ...
            executar_comando "$cmd_substituido -oX $output_file" "Nmap IPv6 (iniciado às $start_time_display)" "$output_file" "Portas escaneadas" "Nenhuma porta encontrada"
